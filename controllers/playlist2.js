...
const getAll = (req, res) => {
    mongodb.getDatabase().db().collection('playlist-2').find()
...
const getSingle = (req, res) => {
    ...
    mongodb.getDatabase().db().collection('playlist-2').find({_id: songId})
...
const addSong = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().db().collection('playlist-2').insertOne(song);
...
const updateSong = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().db().collection('playlist-2').replaceOne({_id: songId}, song);
...
const deleteSong = async (req, res) => {
    ...
    const songId = new ObjectId(req.params.id);
    const response = await mongodb.getDatabase().db().collection('playlist-2').deleteOne({_id: songId});
